...
arima_fn<-function(df){
  xreg1=as.matrix(df[,c(1,5)])      #need to make sure that column order                   xreg = xreg               doesn't get messed up here.
  auto.arima(df$ts) 
}
...
############################## linear models ######################
lm_mod<-function(df){
  lm(data=df,block~hour + weekday + holiday + tmpf + dwpf + p01i + vsby + sknt
    }
...
mods<-mods %>% mutate(lnmods=map(data,lm_mod))
