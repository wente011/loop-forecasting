...
gpks2<-df2 %>% mutate(bkw=bkw1 + bkw2,my=paste0(month(Date),"-",year(Date)),weekly=Date %>% as.POSIXlt %>% (function(x){ y<-x$wday})) %>% group_by(my) %>% nest() %>% mutate(mods=map(data, ~gam(.$bkw ~ s(.$Hour,bs="cr",k=24) + s(.$weekly,bs="ps",k=7),family=gaussian))) %>% mutate(aug=map(mods,~augment(.x))) %>% unnest(aug) %>% ungroup()
...
staticplot<- gpks2 %>% ggplot(aes(x=Date,y=..bkw,group=my)) + geom_point() + geom_line(aes(x=Date,y=.fitted)) +   
